...
local function GetCustomEntries(includeUnavailable, keepWildcardSlots, includeSpecHidden)
    ...
    -- Runtime mirrors custom trackers: keep the wildcard identity in settings,
    -- but resolve it to the currently equipped item before creating an icon.
    for _, entry in ipairs(wildcardEntries) do
        local settings = GetEntrySettings(entry.kind, entry.id)
        if not DB.IsHiddenForCurrentSpec(entry.kind, entry.id) and ItemsData:IsTrackableWildcardSlot(entry.id) then
...
local function SyncVisibility()
    ...
    host:SetShown(ns.db.profile.tracker_enabled and viewer:IsShown() and #icons > 0)
end
...
function EssentialCustomTracker:PrepareLayoutFrame(isHorizontal, fromDirection)
    ...
    if point ~= wall or relativeTo ~= viewer or relativePoint ~= wall or x ~= 0 or y ~= 0 then
        host:ClearAllPoints()
...
function EssentialCustomTracker:Refresh()
    ...
    for index, entry in ipairs(entries) do
        ...
        icon.frame:SetSize(width, height)
...
function EssentialCustomTracker:Initialize()
    ...
    host:SetScript("OnEvent", function(_, event, arg1, _arg2, _arg3, _arg4, arg5)
        ...
        elseif event == "SPELL_UPDATE_COOLDOWN" and arg5 then
