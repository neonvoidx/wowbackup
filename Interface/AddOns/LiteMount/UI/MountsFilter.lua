...
local DROPDOWNS = {
    ...
    },
    {
        disable = ( WOW_PROJECT_ID ~= 1 ),
        text = MODEL,
        checked = function (k) return LM.UIFilter.IsModelChecked(k) end,
        set = function (k, v) LM.UIFilter.SetModelFilter(k, v) end,
        setall = function (v) LM.UIFilter.SetAllModelFilters(v) end,
        menulist = function () return LM.UIFilter.GetModels() end,
        gettext = function (k) return LM.UIFilter.GetModelText(k) end,
    },
    {
        disable = ( WOW_PROJECT_ID ~= 1 ),
        text = SOURCES,
