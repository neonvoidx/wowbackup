...
local function areAuraBarsDisabledForClient()
	return addon.AuraCompat ~= nil
end

local function shouldBuildBar() return not areAuraBarsDisabledForClient() and isShaman() and isUFPlayerEnabled() end
...
local function getMaelstromWeaponStacksFromPlayerAura()
	if areAuraBarsDisabledForClient() then return 0 end
	if not C_UnitAuras then return 0 end

	local aura
	if C_UnitAuras.GetPlayerAuraBySpellID then
		aura = C_UnitAuras.GetPlayerAuraBySpellID(MAELSTROM_WEAPON_SPELL_ID)
	elseif C_UnitAuras.GetUnitAuras then
		for _, data in pairs(C_UnitAuras.GetUnitAuras("player", "HELPFUL") or {}) do
			local spellId = data and data.spellId
			if not (issecretvalue and issecretvalue(spellId)) and spellId == MAELSTROM_WEAPON_SPELL_ID then
				aura = data
				break
			end
		end
...
function ShamanMaelstromWeaponBar:UpdatePower()
	...
	local stacks = min(getMaelstromWeaponStacks(), MAX_STACKS)
