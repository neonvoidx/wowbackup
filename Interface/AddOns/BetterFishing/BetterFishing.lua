...
local internal = {
  ...
  previousClickTime = 0,
  isNotMainline = WOW_PROJECT_ID ~= WOW_PROJECT_MAINLINE,
...
function BetterFishing:RunDeferred()
  for _, task in ipairs(internal.pending) do
    task.obj[task.method](task.obj, unpack(task.args))
  end
  table.wipe(internal.pending)
end

function BetterFishing:Defer(method, ...)
  if not InCombatLockdown() then
    self[method](self, ...)
  else
    table.insert(internal.pending, { obj = self, method = method, args = {...} })
  end
end

function BetterFishing:GetFishingID()
  if internal.isClassicEra or internal.isTBC then
    for fishingID, _ in pairs(FishingIDs) do
      if C_SpellBook.IsSpellKnown(fishingID) then
        return fishingID
      end
    end
  end
...
function BetterFishing:UpdateAttributes()
  if self.secureButton then
    --self.secureButton:SetAttribute("isClassicEra", internal.isClassicEra)
    self.secureButton:SetAttribute("useKeyDowns", GetCVarBool("ActionButtonUseKeyDown"))
  end
end
...
function BetterFishing_Run()
  if InCombatLockdown() or BetterFishing:IsFlying() or GetNumLootItems() ~= 0 or BetterFishing:IsFishing() or (not BetterFishingDB.overrideLunker and BetterFishing:IsLunkerActive()) then return end
...
function BetterFishing:AllowFishing()
  if not C_SpellBook.IsSpellKnown(self:GetFishingID())
  or ((internal.isClassicEra or internal.isTBC) and not self:IsFishingpoleEquipped())
  or IsPlayerMoving()
  or (IsMounted() and not (BetterFishingDB.doubleClickEnabled and BetterFishingDB.doubleClickForce))
...
function BetterFishing:ClearBinds()
...
function BetterFishing:OnEvent(event, ...)
  ...
  elseif event == "GLOBAL_MOUSE_DOWN" then
    ...
    if ... == "RightButton" and not IsMouseButtonDown("LeftButton") and not InCombatLockdown() then
      if GetNumLootItems() == 0 then
        local currentTime = GetTime()
        local lastClick = internal.previousClickTime or 0
        local doubleClickTime = currentTime - lastClick

        -- Debounce
        if ((currentTime - (internal.lastCastTime or 0)) < 0.5) then
          return
        end
        ...
        if (doubleClickTime >= internal.DOUBLECLICK_MIN_SECONDS and doubleClickTime <= BetterFishingDB.doubleClickSpeed) then
          if BetterFishing:AllowFishing() then
            ...
            internal.lastCastTime = currentTime
  ...
  elseif event == "GLOBAL_MOUSE_UP" then
    if internal.isInteractBinding and ... == "RightButton" and not IsMouseButtonDown("LeftButton") and not InCombatLockdown() then
  ...
  elseif event == "UNIT_SPELLCAST_CHANNEL_STOP" then
    local unit,_,spellID = ...
    if unit == "player" and (FishingIDs[spellID]) then
      self:Defer("ClearBinds")
    end
  elseif event == "PLAYER_LOGOUT" then
    BetterFishing:ResetCVars(true)
  elseif event == "CVAR_UPDATE" then
    if ... == "ActionButtonUseKeyDown" then
      self:Defer("UpdateAttributes")
    end
