				...
				if k == "Points" then
					...
					for i = 1, #v do
						...
						if pointsData.Point then
							newPointsData[i].Point = Data.Helpers.getOppositeHorizontalPoint(pointsData.Point)
								or pointsData.Point
						end
						if pointsData.RelativePoint then
							newPointsData[i].RelativePoint = Data.Helpers.getOppositeHorizontalPoint(
								pointsData.RelativePoint
...
function BattleGroundEnemies:RegisterEvents()
	local allEvents =
		Data.Helpers.JoinArrays(self.GeneralEvents, self.ClassicEvents, self.WrathEvents, self.RetailEvents)
	...
	function BattleGroundEnemies:GetPlayerbuttonByUnitID(unitID, playerType, ignoreExistingArena)
		...
		elseif #candidates > 1 then
			...
			if
				okName
				and targetServer
				and not (issecretvalue and issecretvalue(targetServer))
				and targetServer ~= ""
			then
...
function BattleGroundEnemies:ScanTargets()
	...
	for i = 1, maxNameplate do
		...
		elseif UnitExists(targetUnitID) and UnitIsFriend("player", targetUnitID) then
			...
			local allyBtn = targetName
				and self.Allies.Players
				and self:SafeGetPlayerButton(self.Allies.Players, targetName)
		...
		for i = 1, numGroupMembers do
			...
			if name and classToken then
				self.Allies:AddGroupMember(
					name,
					UnitIsGroupLeader(unitID),
					UnitIsGroupAssistant(unitID),
					classToken,
					unitID
				)
				addedCount = addedCount + 1
