...
local function areCDMAurasDisabledForClient()
	return CooldownPanels.AreCDMAurasDisabledForClient and CooldownPanels:AreCDMAurasDisabledForClient() == true
end
...
function cdm.ResetPersistentFrameCaches(runtime)
	...
	if type(runtime.pandemicFrameActiveByFrame) == "table" then
		wipe(runtime.pandemicFrameActiveByFrame)
	else
		runtime.pandemicFrameActiveByFrame = cdm.EnsureWeakKeyTable(nil)
	end
end
...
local function getRuntime()
	...
	if runtime then
		...
		runtime.pandemicFrameEntries = cdm.EnsureWeakKeyTable(runtime.pandemicFrameEntries)
		runtime.pandemicFrameActiveByFrame = cdm.EnsureWeakKeyTable(runtime.pandemicFrameActiveByFrame)
		runtime.hookedPandemicFrames = cdm.EnsureWeakKeyTable(runtime.hookedPandemicFrames)
		runtime.hookedTotemCooldownFrames = cdm.EnsureWeakKeyTable(runtime.hookedTotemCooldownFrames)
	...
	runtime = {
		...
		frameEntries = {},
		pandemicFrameEntries = cdm.EnsureWeakKeyTable(nil),
		hookedFrames = {},
		hookedPandemicFrames = cdm.EnsureWeakKeyTable(nil),
		hookedTotemCooldownFrames = cdm.EnsureWeakKeyTable(nil),
		...
		pandemicFrameActiveByFrame = cdm.EnsureWeakKeyTable(nil),
...
local function resolveAuraStackCount(auraUnit, auraInstanceID, applications)
	...
	if areCDMAurasDisabledForClient() then return displayCount end
...
local function getAuraDataByAuraInstanceIDCached(auraUnit, auraInstanceID)
	if areCDMAurasDisabledForClient() then return nil end
...
function CDMAuras:ScanTrackedBuffs(force, mode)
	local runtime = getRuntime()
	if areCDMAurasDisabledForClient() and mode ~= "import" then
		runtime.scan = { list = {}, byCooldownID = {}, byCooldownKey = {}, bySpellID = {}, sorted = true, fullDerived = true }
...
local function unregisterFrameBinding(runtime, key, frame)
	...
	if pandemicKeys then
		...
		if not next(pandemicKeys) then
			...
			if runtime.pandemicFrameActiveByFrame then runtime.pandemicFrameActiveByFrame[frame] = nil end
		end
	end
...
local function hookFrame(frame)
	if areCDMAurasDisabledForClient() then return end
	...
	if frame.OnPlayerTotemUpdateEvent then hooksecurefunc(frame, "OnPlayerTotemUpdateEvent", function(self) CDMAuras:HandleFrameTotemMutation(self) end) end
	if frame.OnNewTarget then hooksecurefunc(frame, "OnNewTarget", function(self) CDMAuras:HandleFrameTargetChanged(self) end) end
end

local function hookTotemCooldownDoneFrame(frame)
	if areCDMAurasDisabledForClient() then return end
	if not (frame and frame.totemData ~= nil and frame.Cooldown and frame.Cooldown.HookScript) then return end
	local runtime = getRuntime()
	runtime.hookedTotemCooldownFrames = cdm.EnsureWeakKeyTable(runtime.hookedTotemCooldownFrames)
	if runtime.hookedTotemCooldownFrames[frame] then return end
	runtime.hookedTotemCooldownFrames[frame] = true
	frame.Cooldown:HookScript("OnCooldownDone", function()
		RunNextFrame(function() CDMAuras:HandleFrameTotemMutation(frame) end)
	end)
end
...
local function hookPandemicFrame(frame)
	...
	local runtime = getRuntime()
	runtime.hookedPandemicFrames = cdm.EnsureWeakKeyTable(runtime.hookedPandemicFrames)
	if runtime.hookedPandemicFrames[frame] then return end
	runtime.hookedPandemicFrames[frame] = true
...
local function registerFrameBinding(runtime, key, frame)
	...
	keys[key] = true
	hookFrame(frame)
	hookTotemCooldownDoneFrame(frame)
...
local function updatePandemicFrameBinding(runtime, key, frame, wantsPandemic)
	...
	elseif pandemicKeys then
		...
		if not next(pandemicKeys) then
			runtime.pandemicFrameEntries[frame] = nil
			runtime.pandemicFrameActiveByFrame = cdm.EnsureWeakKeyTable(runtime.pandemicFrameActiveByFrame)
			runtime.pandemicFrameActiveByFrame[frame] = nil
		end
	end
end
...
function CDMAuras:HasActiveTrackedPanels()
	if areCDMAurasDisabledForClient() then return false end
...
function CDMAuras:UpdateEventRegistration()
	...
	if not self.eventsRegistered then
		...
		-- Same-instance application changes do not always trigger CDM Set/Clear hooks.
			...
			if not entry or entry.type ~= ENTRY_TYPE then
				...
				elseif frameMatchesTrackedSpell then
					...
					if state.pandemicActive ~= nextPandemicActive then
						...
						runtime.pandemicFrameActiveByFrame = cdm.EnsureWeakKeyTable(runtime.pandemicFrameActiveByFrame)
						runtime.pandemicFrameActiveByFrame[frame] = nextPandemicActive and true or nil
						changed = true
					end
...
function CDMAuras:HandleFrameTotemMutation(frame)
	...
	RunNextFrame(function() CDMAuras:ApplyFrameTotemMutation(frame) end)
end
...
function CDMAuras:HandleFramePandemicStateChanged(frame, isActive)
	...
	runtime.pandemicFrameActiveByFrame = cdm.EnsureWeakKeyTable(runtime.pandemicFrameActiveByFrame)
	if not pandemicActive and runtime.pandemicFrameActiveByFrame[frame] ~= true then return end
	runtime.pandemicFrameActiveByFrame[frame] = pandemicActive and true or nil
	for key in pairs(keys) do
		local state = runtime.entryStates[key]
		if state then
...
function CDMAuras:ImportEntries(panelId, sourceKind)
	...
	for _, entry in pairs(panel.entries) do
		if entry and entry.type == ENTRY_TYPE then
			...
			local spellID = tonumber(entry.auraSpellID or entry.spellID)
			if spellID then existingBySpellID[spellID] = true end
		end
	end

	local list = self:ScanTrackedBuffs(true, "import")
	...
	for _, info in ipairs(list or {}) do
		...
		if info and info.availableSources and info.availableSources[sourceType] and sourceFrame then
			...
			local spellID = tonumber(info.spellID)
			if not isValidCooldownID(info.cooldownID) or not spellID then
				stats.invalid = stats.invalid + 1
			else
				local cooldownKey = tostring(info.cooldownID)
				if existingByCooldownID[cooldownKey] or existingBySpellID[spellID] then
					...
					if entry and tonumber(entry.auraSpellID or entry.spellID) then
						entry.id = entryId
...
function CDMAuras:SyncEntries(panelId, sourceKind)
	...
	local wantedByCooldownID, wantedOrder = {}, {}
	local stats = { added = 0, removed = 0, updated = 0, invalid = 0, seen = 0, sourceLabel = sourceLabel }
	local list = self:ScanTrackedBuffs(true, "import")
	for _, info in ipairs(list or {}) do
		local sourceFrame = info and info.iconFrame or nil
		local spellID = info and tonumber(info.spellID) or nil
		if info and info.availableSources and info.availableSources[SOURCE_ICON] and sourceFrame then
			stats.seen = stats.seen + 1
			if not isValidCooldownID(info.cooldownID) or not spellID then
				stats.invalid = stats.invalid + 1
			else
	...
	local existingByCooldownID, otherOrder = {}, {}
	for _, entryId in ipairs(panel.order) do
		local entry = panel.entries[entryId]
		local cooldownKey = entry and entry.type == ENTRY_TYPE and isValidCooldownID(entry.cooldownID) and tostring(entry.cooldownID) or nil
	...
	for entryId, entry in pairs(panel.entries) do
		if entry and entry.cdmSyncManaged == true then
			local cooldownKey = entry.type == ENTRY_TYPE and isValidCooldownID(entry.cooldownID) and tostring(entry.cooldownID) or nil
...
function CDMAuras:BuildRuntimeData(panelId, entryId, entry, entryLayout, alwaysShowMode)
	...
	if hasTotemData then hookTotemCooldownDoneFrame(chosenFrame) end
	...
	if chosenFrame then
		runtime.pandemicFrameActiveByFrame = cdm.EnsureWeakKeyTable(runtime.pandemicFrameActiveByFrame)
		runtime.pandemicFrameActiveByFrame[chosenFrame] = pandemicActive and true or nil
	end
