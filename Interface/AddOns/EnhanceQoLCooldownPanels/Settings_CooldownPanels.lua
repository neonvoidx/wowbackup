...
local function registerDynamicAnchoringSettings()
	...
	local function candidateCanMatchWidth(candidate)
		return candidate and dynamicAnchors:CanMatchRelativeWidth(candidate.targetId) or false
	end
	...
	for index = 1, dynamicAnchors.MAX_CANDIDATES do
		...
		registerCandidateControl({
			...
			setValue = function(value)
				...
				if candidate then
					candidate.targetId = value
					if not candidateCanMatchWidth(candidate) then
						candidate.matchRelativeWidth = false
						candidate.matchRelativeWidthOffset = 0
					end
				end
				notifyChanged(false)
			end,
		...
		registerCandidateControl({
			...
			getValue = function()
				local candidate = getCandidate(index, false)
				return candidateCanMatchWidth(candidate) and candidate.matchRelativeWidth == true or false
			end,
			setValue = function(value)
				local candidate = getCandidate(index, true)
				if candidate then
					local canMatchWidth = candidateCanMatchWidth(candidate)
					candidate.matchRelativeWidth = canMatchWidth and value == true
					if not canMatchWidth then candidate.matchRelativeWidthOffset = 0 end
				end
				notifyChanged(false)
			end,
			isEnabled = function()
				local candidate = getCandidate(index, false)
				return candidateCanMatchWidth(candidate)
			end,
			default = false,
			order = 55,
		})
		registerCandidateControl({
			...
			getValue = function()
				local candidate = getCandidate(index, false)
				return candidateCanMatchWidth(candidate) and tonumber(candidate.matchRelativeWidthOffset) or 0
			end,
			setValue = function(value)
				local candidate = getCandidate(index, true)
				if candidateCanMatchWidth(candidate) then candidate.matchRelativeWidthOffset = tonumber(value) or 0 end
				notifyChanged(false)
			end,
			isEnabled = function()
				local candidate = getCandidate(index, false)
				return candidateCanMatchWidth(candidate) and candidate.matchRelativeWidth == true
			end,
			default = 0,
			order = 57,
		})
