...
Helper.ENTRY_DEFAULTS = {
	...
	cdmAuraOverlayColor = Helper.ACTIVATION_OVERLAY_COLOR_DEFAULT,
	activationOverlayBehaviorUseGlobal = true,
	activationOverlayReverse = true,
...
function Helper.UpdatePanelEntrySpecFilterState(panel)
	if type(panel) ~= "table" or type(panel.entries) ~= "table" then return end
	Helper.entrySpecFilterPanels = Helper.entrySpecFilterPanels or setmetatable({}, { __mode = "k" })
	Helper.entrySpecFilterGenerations = Helper.entrySpecFilterGenerations or setmetatable({}, { __mode = "k" })
	for _, entry in pairs(panel.entries) do
		local filter = entry and entry.specFilter
		if type(filter) == "table" then
			for _, enabled in pairs(filter) do
				if enabled == true then
					Helper.entrySpecFilterPanels[panel] = true
					Helper.entrySpecFilterGenerations[panel] = (Helper.entrySpecFilterGenerations[panel] or 0) + 1
					return
				end
			end
		end
	end
	Helper.entrySpecFilterPanels[panel] = nil
	Helper.entrySpecFilterGenerations[panel] = (Helper.entrySpecFilterGenerations[panel] or 0) + 1
end

function Helper.PanelHasEntrySpecFilter(panel)
	return Helper.entrySpecFilterPanels and Helper.entrySpecFilterPanels[panel] == true or false
end

function Helper.GetPanelEntrySpecFilterGeneration(panel)
	return Helper.entrySpecFilterGenerations and Helper.entrySpecFilterGenerations[panel] or 0
end
...
function Helper.NormalizePanel(panel, defaults)
	...
	Helper.UpdatePanelEntrySpecFilterState(panel)
...
function Helper.NormalizeEntry(entry, defaults)
	...
	if type(entry.specFilter) == "table" then
		...
		for specId, enabled in pairs(entry.specFilter) do
			...
			if normalizedSpecId and normalizedSpecId > 0 and enabled == true then
				cleanedSpecFilter = cleanedSpecFilter or {}
				cleanedSpecFilter[normalizedSpecId] = true
			end
		end
		entry.specFilter = cleanedSpecFilter
	else
	...
	if entry.type == "CDM_AURA" or entry.type == "STANCE" then
		...
		entry.activationOverlayBehaviorUseGlobal = true
		...
		if not hadActivationOverlayBehaviorUseGlobal then
			local hasLegacyActivationOverride = (hadActivationOverlayReverse and entry.activationOverlayReverse == false)
				...
				or (hadActivationOverlayGlow and entry.activationOverlayGlow == true)
			entry.activationOverlayBehaviorUseGlobal = not hasLegacyActivationOverride
...
function Helper.CreatePanel(name, defaults)
	...
	return {
		...
		entries = {},
		order = {},
		hideUnavailableEntriesInEditor = true,
	}
end
