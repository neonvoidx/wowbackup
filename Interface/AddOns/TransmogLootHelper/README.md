# Transmog Loot Helper
A World of Warcraft addon. Download locations: [CurseForge](https://www.curseforge.com/wow/addons/transmog-loot-helper) | [Wago](https://addons.wago.io/addons/tlh)

[![](https://media.forgecdn.net/attachments/1439/398/buymeacoffee-png.png)](https://www.buymeacoffee.com/Slackluster)

**Transmog Loot Helper (TLH)** shows icons for unlearned transmog, mounts, pets, toys, recipes, decor, and more. These show on:
- Bags and bank
- (Black Market) Auction House
- Mailbox
- Loot window and loot rolls
- Vendors
- Profession window
- World quests and quest rewards
- Encounter Journal
- Great Vault

![](https://media.forgecdn.net/attachments/description/1033752/description_17172ffd-6371-4979-a00d-b4da2bca0fa6.png)

There are several styles of icons to choose from!

![](https://media.forgecdn.net/attachments/1526/743/styles-png.png)

It also features a loot tracker, showing you transmog looted by your fellow party/raid members that you still need, and features icons to show items' collection status. Alt+clicking lets you whisper them to ask for the item!

![](https://media.forgecdn.net/attachments/description/1033752/description_62349c27-183a-4589-9013-d193e7a7a519.png)

Currently supported addons:
- ArkInventory
- Baganator
- Bagforge
- BetterBags
- World Quest Tab
- World Quests List
- WoWNotes Bags

## If you are experiencing issues
- Check if the issue occurs with only this addon enabled, or if the issue is a feature you can disable from the addon's settings menu.
- If you run into any issues or if you have other feedback, feel free to join the [Discord](https://discord.gg/FwWZZ39kPX) and ask in the `#transmog-loot-helper` channel, or create a [ticket on GitHub](https://github.com/Slackluster/TransmogLootHelper/issues). :)
