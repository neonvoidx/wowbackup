...
function TAV_DisplayContainerMixin:OnLoad()
	...
	self.backgroundColor = CreateColor(0.25, 0.25, 0.25, 1)
...
function TAV_DisplayContainerMixin:ApplySettings(settings)
	TAV_ControlsPanel.ToggleBordersButton.Icon:SetTexture(
		settings.passiveBorders and "Interface/LFGFRAME/BattlenetWorking9" or "Interface/LFGFRAME/BattlenetWorking4"
	)
...
function TAV_DisplayContainerMixin:OnSearchChanged()
	...
	if not pcall(function()
		TAV:UpdateDisplayList(text, self.enablePatterns)
	end) then
...
function TAV_DisplayContainerMixin:NameMatchesCurrentSearch(name)
	if not name or not self.searchString or self.searchString == "" then
		return false
	end
...
function TAV_DisplayContainerMixin:ToggleBorders()
	...
	TAV_ControlsPanel.ToggleBordersButton.Icon:SetTexture(
		TAV.settings.passiveBorders and "Interface/LFGFRAME/BattlenetWorking9" or "Interface/LFGFRAME/BattlenetWorking4"
	)
	self:UpdateOverlays()
end
...
function TAV_DisplayContainerMixin:OnDragStart()
	if not self.Child:IsShown() then
		return
	end
...
function TAV_DisplayContainerMixin:ShowAtlasInfo(name, atlasInfo)
	if not name or not atlasInfo then
		return
	end
