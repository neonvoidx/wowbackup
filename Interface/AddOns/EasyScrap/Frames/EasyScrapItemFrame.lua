...
for i = 1, 3 do
    ...
    if i == 1 then
        button:SetPoint('BOTTOMLEFT', 3, -45) 
    elseif i == 2 then
        button:SetPoint('BOTTOM', 0, -45) 
    else
        button:SetPoint('BOTTOMRIGHT', -3, -45) 
    end
    ...
    function button:SetCount(count)
        if self.state == 1 then
            if count > 0 then
                EASYSCRAP_AUTOCAST_START(itemFrame.tabButtons[1].shineFrame)
            else
                EASYSCRAP_AUTOCAST_STOP(itemFrame.tabButtons[1].shineFrame)
            end
        end
