...
function EasyScrap:itemScrappable(itemID)
   if self.itemCache[itemID] ~= nil then
      ...
      if GetItemInfo(itemID) then
         ...
         for i = self.tooltipReader:NumLines(), 1, -1 do
            if i >= 1 then
...
function EasyScrap:generateFilterDropdown()
    ...
    EasyScrapFilterSelectionMenu:SetupMenu(function(dropdown, rootDescription)
        for k,v in pairs(EasyScrap.filterSelectionMenuTable) do
            rootDescription:CreateButton(v.text, function() v.func() EasyScrapFilterSelectionMenu:SetDefaultText(v.text) end) 
        end
    end)
    
    if self.activeFilterID == 0 then
        --UIDropDownMenu_SetText(EasyScrapFilterSelectionMenu, "Default")
        EasyScrapFilterSelectionMenu:SetDefaultText("Default")
    else
        --UIDropDownMenu_SetText(EasyScrapFilterSelectionMenu, EasyScrap.saveData.customFilters[self.activeFilterID].name)
        EasyScrapFilterSelectionMenu:SetDefaultText(EasyScrap.saveData.customFilters[self.activeFilterID].name)
    end
end
