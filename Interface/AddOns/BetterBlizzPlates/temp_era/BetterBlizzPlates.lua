...
local function SetCVarsOnLogin()
    if BetterBlizzPlatesDB.hasSaved and not BetterBlizzPlatesDB.disableCVarForceOnLogin then
        ...
        if BetterBlizzPlatesDB.ShowClassColorInNameplate == "1" then
            C_CVar.SetCVar("ShowClassColorInNameplate", "1")
        end
...
local function SetBarWidth(frame, width, useOffsets)
    ...
    if useOffsets then
        if BetterBlizzPlatesDB.classicNameplates then
            ...
            frame.healthBar:SetPoint("LEFT", frame, "LEFT", -width, 0)
            frame.healthBar:SetPoint("RIGHT", frame, "RIGHT", width,0)

            frame.CastBar:SetPoint("LEFT", frame, "LEFT", -width, 0)
            frame.CastBar:SetPoint("RIGHT", frame, "RIGHT", width, 0)
        end
    else
        if BetterBlizzPlatesDB.classicNameplates then
...
local function CreateBetterClassicCastbarBorders(frame)
    ...
    if not frame.CastBar.eventsHooked then
        ...
        frame.CastBar.Icon:SetPoint("RIGHT", frame.CastBar, "LEFT", BetterBlizzPlatesDB.castBarIconXPos-2, BetterBlizzPlatesDB.castBarIconYPos)
    end
...
local function CreateBetterRetailCastbar(frame)
    ...
    frame.CastBar.Flash:Hide()
    frame.CastBar.Flash:SetAlpha(0)
...
local function UpdateSpark()
    ...
    -- if there's no valid range, hide the spark (or just return)
    if range <= 0 then
        return
    end
...
local function HandleNamePlateAdded(unit)
    ...
    if not frame.CastBar then
        frame.CastBar = CreateFrame("StatusBar", nil, frame, "SmallCastingBarFrameTemplate")
        frame.CastBar:Hide()
        -- frame.CastBar.Border = frame.CastBar:CreateTexture()
        -- frame.CastBar.BorderShield = frame.CastBar:CreateTexture()
        -- frame.CastBar.Icon = frame.CastBar:CreateTexture()
        CastingBarFrame_OnLoad(frame.CastBar, unit)
    end
    frame.CastBar.unit = unit
    CastingBarFrame_SetUnit(frame.CastBar, unit)
    ...
    -- Show totem icons
    if config.totemIndicator and info.isNpc then BBP.ApplyTotemIconsAndColorNameplate(frame, unit) end
...
function BBP.CheckAndFetchMissingCVars()
    local db = BetterBlizzPlatesDB
    local missing = false
    for _, cvar in ipairs(cvarList) do
        if db[cvar] == nil then
...
function BBP.TryFetchCVarsUntilReady()
    local db = BetterBlizzPlatesDB
    local ticker
    ticker = C_Timer.NewTicker(1.5, function()
        if BBP.variablesLoaded then
            BBP.CheckAndFetchMissingCVars()
            db.eraCvarOopsieFixed = true
            ticker:Cancel()
        end
    end)
end
...
First:SetScript("OnEvent", function(_, event, addonName)
    if event == "ADDON_LOADED" and addonName then
        if addonName == "BetterBlizzPlates" then
            ...
            if not db.eraCvarOopsieFixed then
                BBP.TryFetchCVarsUntilReady()
            end
                ...
                if isChanneled then
                    ...
                    if currentValue >= maxValue then
                        ...
                        if castType == "uninterruptible" then
                            ...
                            castBar.castText:SetText("Uninterruptible")
                            if showCastBarIconWhenNoninterruptible then
                                castBar.Icon:SetTexture(GetSpellTexture(71))
                        ...
                        elseif castType == "channeled" then
                            ...
                            castBar.castText:SetText("Drain Life")
                            castBar.Icon:Show()
                            castBar.Icon:SetTexture(GetSpellTexture(689))
                            if useCustomCastbarTexture or castBarRecolor then
                                local castBarTexture = castBar:GetStatusBarTexture()
                                castBarTexture:SetDesaturated(true)
