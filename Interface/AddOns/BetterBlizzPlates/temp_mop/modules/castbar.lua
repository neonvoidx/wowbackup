...
local function GetCastbarTargetSuffix(unit)
    ...
    if BetterBlizzPlatesDB.targetTextTestMode then
        ...
        return GetColoredTargetString(GetUnitName("player"), classIdentifier)
    end
...
function BBP.SetCastbarSpellText(castBar, spellName, unit)
    if not spellName then return end
    if BetterBlizzPlatesDB.showNameplateTargetText and BetterBlizzPlatesDB.castbarTargetTextInsideBar then
        local coloredName = GetCastbarTargetSuffix(unit or castBar.unit)
        if coloredName then
            castBar.Text:SetText(spellName .. ": " .. coloredName)
            return
        end
    end
...
function BBP.CastbarOnEvent(frame, event)
    ...
    if name then
        BBP.SetCastbarSpellText(castBar, name, unit)
    end
