...
local DYNAMIC_ANCHOR_ID = "bar:experience"
...
local DB_TEXT_LEFT_OFFSET_X = "xpBarTextLeftOffsetX"
local DB_TEXT_LEFT_OFFSET_Y = "xpBarTextLeftOffsetY"
local DB_TEXT_CENTER_OFFSET_X = "xpBarTextCenterOffsetX"
local DB_TEXT_CENTER_OFFSET_Y = "xpBarTextCenterOffsetY"
local DB_TEXT_RIGHT_OFFSET_X = "xpBarTextRightOffsetX"
local DB_TEXT_RIGHT_OFFSET_Y = "xpBarTextRightOffsetY"
...
local DB_VISIBILITY = "xpBarVisibility"
local DB_VISIBILITY_EXPLICIT = "xpBarVisibilityExplicit"
...
function ExperienceBar:GetVisibilityRuntimeConfig()
...
function ExperienceBar:RegisterVisibilityFrame()
	...
	rb.RegisterCustomVisibilityFrame("XP", self.frame, self:GetVisibilityRuntimeConfig())
end

function ExperienceBar:ApplyVisibilityPreference(runtimeVisible)
	if self.frame then self.frame._rbDesiredVisible = runtimeVisible == true end
	self:RegisterVisibilityFrame()
	local rb = addon.Aura and addon.Aura.ResourceBars
	if rb and rb.ApplyVisibilityPreference then rb.ApplyVisibilityPreference("xpBar") end
end
...
function ExperienceBar:GetTextLeftOffsetX() return normalizeTextOffset(getValue(DB_TEXT_LEFT_OFFSET_X, defaults.textLeftOffsetX), defaults.textLeftOffsetX) end

function ExperienceBar:GetTextLeftOffsetY() return normalizeTextOffset(getValue(DB_TEXT_LEFT_OFFSET_Y, defaults.textLeftOffsetY), defaults.textLeftOffsetY) end

function ExperienceBar:GetTextCenterOffsetX() return normalizeTextOffset(getValue(DB_TEXT_CENTER_OFFSET_X, defaults.textCenterOffsetX), defaults.textCenterOffsetX) end

function ExperienceBar:GetTextCenterOffsetY() return normalizeTextOffset(getValue(DB_TEXT_CENTER_OFFSET_Y, defaults.textCenterOffsetY), defaults.textCenterOffsetY) end

function ExperienceBar:GetTextRightOffsetX() return normalizeTextOffset(getValue(DB_TEXT_RIGHT_OFFSET_X, defaults.textRightOffsetX), defaults.textRightOffsetX) end

function ExperienceBar:GetTextRightOffsetY() return normalizeTextOffset(getValue(DB_TEXT_RIGHT_OFFSET_Y, defaults.textRightOffsetY), defaults.textRightOffsetY) end
...
function ExperienceBar:ApplyTextAnchors()
	if not self.frame then return end
...
function ExperienceBar:ApplyBlizzardTrackingVisibility()
	...
	local rb = addon.Aura and addon.Aura.ResourceBars
	if rb and rb.SetBlizzardTrackingKilled then rb.SetBlizzardTrackingKilled("xpBar", hide) end
end
...
function ExperienceBar:ApplyLayoutData(data)
	...
	local anchorRelativeFrame = normalizeAnchorRelativeFrame(data.anchorRelativeFrame or data.anchorTarget or addon.db.xpBarAnchorTarget or defaults.anchorRelativeFrame)
	local anchorMatchWidth = addon.db.xpBarAnchorMatchWidth == true
	...
	local anchorMatchWidthOffset = clamp(data.anchorMatchWidthOffset or data.anchorMatchRelativeWidthOffset or addon.db.xpBarAnchorMatchWidthOffset or defaults.anchorMatchRelativeWidthOffset or 0, -200, 200)
	local anchorPoint = normalizeAnchorPoint(data.point or addon.db.xpBarAnchorPoint, defaults.anchorPoint)
	local anchorRelativePoint = normalizeAnchorPoint(data.relativePoint or addon.db.xpBarAnchorRelativePoint, anchorPoint)
	local anchorOffsetX = normalizeAnchorOffset(data.x ~= nil and data.x or addon.db.xpBarAnchorOffsetX, defaults.anchorOffsetX)
	local anchorOffsetY = normalizeAnchorOffset(data.y ~= nil and data.y or addon.db.xpBarAnchorOffsetY, defaults.anchorOffsetY)
	local textEnabled = addon.db.xpBarShowText ~= false
	if data.textEnabled ~= nil then textEnabled = data.textEnabled == true end
	local textLeftMode = normalizeTextContentMode(data.textLeftMode or addon.db.xpBarTextLeftMode or defaults.textLeftMode)
	local textCenterMode = normalizeTextContentMode(data.textCenterMode or data.textMode or addon.db.xpBarTextCenterMode or addon.db.xpBarTextMode or defaults.textCenterMode or defaults.textMode)
	local textRightMode = normalizeTextContentMode(data.textRightMode or addon.db.xpBarTextRightMode or defaults.textRightMode)
	local textLeftOffsetX = normalizeTextOffset(data.textLeftOffsetX ~= nil and data.textLeftOffsetX or addon.db.xpBarTextLeftOffsetX, defaults.textLeftOffsetX)
	local textLeftOffsetY = normalizeTextOffset(data.textLeftOffsetY ~= nil and data.textLeftOffsetY or addon.db.xpBarTextLeftOffsetY, defaults.textLeftOffsetY)
	local textCenterOffsetX = normalizeTextOffset(data.textCenterOffsetX ~= nil and data.textCenterOffsetX or addon.db.xpBarTextCenterOffsetX, defaults.textCenterOffsetX)
	local textCenterOffsetY = normalizeTextOffset(data.textCenterOffsetY ~= nil and data.textCenterOffsetY or addon.db.xpBarTextCenterOffsetY, defaults.textCenterOffsetY)
	local textRightOffsetX = normalizeTextOffset(data.textRightOffsetX ~= nil and data.textRightOffsetX or addon.db.xpBarTextRightOffsetX, defaults.textRightOffsetX)
	local textRightOffsetY = normalizeTextOffset(data.textRightOffsetY ~= nil and data.textRightOffsetY or addon.db.xpBarTextRightOffsetY, defaults.textRightOffsetY)
	local textSize = clamp(data.textSize or addon.db.xpBarTextSize or defaults.textSize, TEXT_SIZE_MIN, TEXT_SIZE_MAX)
	local textFont = data.textFont or addon.db.xpBarTextFont or defaults.textFont or "DEFAULT"
	local textOutline = normalizeTextOutline(data.textOutline or addon.db.xpBarTextOutline or defaults.textOutline)
	local textR, textG, textB, textA = normalizeColor(data.textColor or addon.db.xpBarTextColor or defaults.textColor, defaults.textColor)
	local abbreviateNumbers = addon.db.xpBarTextAbbreviateNumbers == true
	if data.abbreviateNumbers ~= nil then abbreviateNumbers = data.abbreviateNumbers == true end
	local hideInPetBattle = addon.db.xpBarHideInPetBattle == true
	if data.hideInPetBattle ~= nil then hideInPetBattle = data.hideInPetBattle == true end
	local hideBlizzardTracking = addon.db.xpBarHideBlizzardTracking == true
	if data.hideBlizzardTracking ~= nil then hideBlizzardTracking = data.hideBlizzardTracking == true end
	local restedOverlayEnabled = addon.db.xpBarRestedOverlay ~= false
	if data.restedOverlayEnabled ~= nil then restedOverlayEnabled = data.restedOverlayEnabled == true end

	addon.db.xpBarWidth = width
	addon.db.xpBarHeight = height
	addon.db.xpBarTexture = texture
	addon.db.xpBarColor = { r = r, g = g, b = b, a = a }
	addon.db.xpBarRestedColor = { r = rr, g = rg, b = rb, a = ra }
	addon.db.xpBarBackgroundEnabled = bgEnabled
	addon.db.xpBarBackgroundTexture = bgTexture
	addon.db.xpBarBackgroundColor = { r = bgr, g = bgg, b = bgb, a = bga }
	addon.db.xpBarBorderEnabled = borderEnabled
	addon.db.xpBarBorderTexture = borderTexture
	addon.db.xpBarBorderColor = { r = bdr, g = bdg, b = bdb, a = bda }
	addon.db.xpBarBorderSize = borderSize
	addon.db.xpBarBorderOffset = borderOffset
	addon.db.xpBarFillDirection = fillDirection
	local prevAnchorRelativeFrame = addon.db.xpBarAnchorTarget
	addon.db.xpBarAnchorTarget = anchorRelativeFrame
	addon.db.xpBarAnchorMatchWidth = anchorMatchWidth and true or false
	addon.db.xpBarAnchorMatchWidthOffset = anchorMatchWidthOffset
	addon.db.xpBarAnchorPoint = anchorPoint
	addon.db.xpBarAnchorRelativePoint = anchorRelativePoint
	addon.db.xpBarAnchorOffsetX = anchorOffsetX
	addon.db.xpBarAnchorOffsetY = anchorOffsetY
	addon.db.xpBarShowText = textEnabled and true or false
	addon.db.xpBarTextLeftMode = textLeftMode
	addon.db.xpBarTextCenterMode = textCenterMode
	addon.db.xpBarTextRightMode = textRightMode
	addon.db.xpBarTextLeftOffsetX = textLeftOffsetX
	addon.db.xpBarTextLeftOffsetY = textLeftOffsetY
	addon.db.xpBarTextCenterOffsetX = textCenterOffsetX
	addon.db.xpBarTextCenterOffsetY = textCenterOffsetY
	addon.db.xpBarTextRightOffsetX = textRightOffsetX
	addon.db.xpBarTextRightOffsetY = textRightOffsetY
	addon.db.xpBarTextMode = normalizeLegacyTextMode(textCenterMode)
	addon.db.xpBarTextSize = textSize
	addon.db.xpBarTextFont = textFont
	addon.db.xpBarTextOutline = textOutline
	addon.db.xpBarTextColor = { r = textR, g = textG, b = textB, a = textA }
	addon.db.xpBarTextAbbreviateNumbers = abbreviateNumbers and true or false
	addon.db.xpBarHideInPetBattle = hideInPetBattle and true or false
	addon.db.xpBarHideBlizzardTracking = hideBlizzardTracking and true or false
	addon.db.xpBarRestedOverlay = restedOverlayEnabled and true or false
...
local function applySetting(field, value)
	...
	elseif field == "anchorRelativeFrame" then
		...
		if prev ~= target then
			...
			addon.db.xpBarAnchorPoint = point
			addon.db.xpBarAnchorRelativePoint = relPoint
			addon.db.xpBarAnchorOffsetX = x
			addon.db.xpBarAnchorOffsetY = y
	...
	elseif field == "anchorMatchWidth" then
		local enabled = value == true
		addon.db.xpBarAnchorMatchWidth = enabled and true or false
		value = enabled
		refreshSettingsUI()
	elseif field == "anchorMatchWidthOffset" then
		local offset = clamp(value, -200, 200)
		addon.db.xpBarAnchorMatchWidthOffset = offset
		value = offset
	elseif field == "anchorPoint" then
		...
		editField = "point"
		value = point
		local rel = normalizeAnchorPoint(addon.db.xpBarAnchorRelativePoint, point)
		if addon.db.xpBarAnchorRelativePoint ~= rel then
			addon.db.xpBarAnchorRelativePoint = rel
...
function ExperienceBar:RegisterEditMode(frame)
	...
	if SettingType then
		...
		local function setVisibilityRule(rule, state)
			...
			ExperienceBar:ApplyVisibilityPreference(ExperienceBar.frame and ExperienceBar.frame._rbDesiredVisible ~= false)
		end
		...
		settings = {
			...
			},
			{
				...
				valueStep = 1,
				allowInput = true,
				get = function() return ExperienceBar:GetTextLeftOffsetX() end,
					set = function(_, value) applySetting("textLeftOffsetX", value) end,
					formatter = function(value) return tostring(math.floor((tonumber(value) or 0) + 0.5)) end,
					isShown = function() return ExperienceBar:GetTextLeftMode() ~= "NONE" end,
					isEnabled = function() return ExperienceBar:GetTextEnabled() end,
				},
			{
				...
				valueStep = 1,
				allowInput = true,
				get = function() return ExperienceBar:GetTextLeftOffsetY() end,
				...
				valueStep = 1,
				allowInput = true,
				get = function() return ExperienceBar:GetTextCenterOffsetX() end,
					set = function(_, value) applySetting("textCenterOffsetX", value) end,
					formatter = function(value) return tostring(math.floor((tonumber(value) or 0) + 0.5)) end,
					isShown = function() return ExperienceBar:GetTextCenterMode() ~= "NONE" end,
					isEnabled = function() return ExperienceBar:GetTextEnabled() end,
				},
			{
				...
				valueStep = 1,
				allowInput = true,
				get = function() return ExperienceBar:GetTextCenterOffsetY() end,
				...
				valueStep = 1,
				allowInput = true,
				get = function() return ExperienceBar:GetTextRightOffsetX() end,
					set = function(_, value) applySetting("textRightOffsetX", value) end,
					formatter = function(value) return tostring(math.floor((tonumber(value) or 0) + 0.5)) end,
					isShown = function() return ExperienceBar:GetTextRightMode() ~= "NONE" end,
					isEnabled = function() return ExperienceBar:GetTextEnabled() end,
				},
			{
				...
				valueStep = 1,
				allowInput = true,
				get = function() return ExperienceBar:GetTextRightOffsetY() end,
	...
	EditMode:RegisterFrame(EDITMODE_ID, {
		...
		onEnter = function()
			ExperienceBar:ShowEditModeHint(true)
			if addon.DynamicAnchors and addon.DynamicAnchors:IsFrameAssignmentEnabled(DYNAMIC_ANCHOR_ID) then
				RunNextFrame(function()
					if ExperienceBar.previewing then ExperienceBar:ApplyDynamicAnchor() end
				end)
			end
		end,
		...
		settings = settings,
		relativeTo = function()
			local winner = ExperienceBar:GetDynamicAnchorWinner()
			return winner and winner.frame or ExperienceBar:ResolveAnchorFrame()
		end,
		allowDrag = function()
			return not (addon.DynamicAnchors and addon.DynamicAnchors:IsFrameAssignmentEnabled(DYNAMIC_ANCHOR_ID)) and ExperienceBar:AnchorUsesUIParent()
		end,
	...
	if enabled then
		if addon.DynamicAnchors then
			addon.DynamicAnchors:RegisterSimpleFrame({
				...
				label = L["ExperienceBar"] or "Experience Bar",
				menuGroup = "RESOURCE_BARS",
				menuGroupLabel = L["Resource Bars"],
				menuGroupOrder = 200,
				getFrame = function() return ExperienceBar.frame end,
				isAvailable = function(frame) return ExperienceBar:IsEnabled() and frame ~= nil and frame:IsShown() end,
				apply = function()
					ExperienceBar:ApplyDynamicAnchor()
					ExperienceBar:ApplySize()
				end,
