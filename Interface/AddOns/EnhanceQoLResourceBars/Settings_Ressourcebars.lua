...
registerEditModeBars = function()
	...
	local function registerBar(idSuffix, frameName, barType, widthDefault, heightDefault, opts)
		...
		local function collectSharedPowerTypes()
			...
			if not genericSharedPowerEditor then return liveType and isPowerTypeAvailableForSettings(liveType) and { liveType } or {} end
			if ResourceBars and ResourceBars.GetSharedSlotPossibleTypes then
				...
				for _, pType in ipairs(ResourceBars.GetSharedSlotPossibleTypes(sharedSlot, addon.variables.unitClass) or {}) do
					if pType and isPowerTypeAvailableForSettings(pType) then wanted[pType] = true end
				end
				if liveType and isPowerTypeAvailableForSettings(liveType) then wanted[liveType] = true end
				for _, pType in ipairs(ResourceBars.classPowerTypes or {}) do
			...
			local seen = {}
			for _, pType in ipairs(ResourceBars.classPowerTypes or {}) do
				if wanted[pType] and isPowerTypeAvailableForSettings(pType) and not seen[pType] then
					out[#out + 1] = pType
					seen[pType] = true
				end
			end
			for pType in pairs(wanted) do
				if isPowerTypeAvailableForSettings(pType) and not seen[pType] then out[#out + 1] = pType end
			end
			return out
		end
		...
		local function currentEditorSupportsThresholdLines()
			local pType = currentEditorPowerType()
			if ResourceBars and ResourceBars.SupportsThresholdLines then return ResourceBars.SupportsThresholdLines(pType) == true end
			return pType ~= "HEALTH" and not (ResourceBars and ResourceBars.separatorEligible and pType and ResourceBars.separatorEligible[pType] == true)
		end
		...
		if settingType then
			...
			-- Segment spacing controls (eligible bars only)
			...
			-- Threshold line controls are only useful for continuous bars.
			if barType ~= "HEALTH" or genericSharedPowerEditor then
				settingsList[#settingsList + 1] = {
					...
					hasOpacity = true,
					isShown = currentEditorSupportsThresholdLines,
					parentId = "frame",
				}
				...
				settingsList[#settingsList + 1] = {
					...
					isEnabled = function()
						local c = curSpecCfg()
						return currentEditorSupportsThresholdLines() and c and c.showThresholds == true
					end,
					isShown = currentEditorSupportsThresholdLines,
					parentId = "frame",
				}
				...
				for i = 1, #thresholdLabels do
					settingsList[#settingsList + 1] = {
						...
						isShown = function() return currentEditorSupportsThresholdLines() and i <= thresholdCount() end,
						parentId = "frame",
					}
				end
			end
...
local function ensureResourceBarsSuiteSection(cat)
	...
	expandable = addon.functions.SettingsCreateExpandableSection(cat, {
		...
		description = L["ResourceBarsSuiteDesc"]
			or "Configure custom resource, experience, reputation, honor and absorb bars.",
		expanded = false,
		colorizeTitle = false,
		iconKey = "resource",
...
local function createProgressBarSettings(cat, expandable)
	...
	addon.functions.SettingsCreateText(cat, "|cffffd700" .. (L["xpBarEditModeHint"] or "Configure anchor, style, and text in Edit Mode.") .. "|r", {
		parentSection = expandable,
	})

	addon.functions.SettingsCreateHeadline(cat, L["ReputationBar"] or "Reputation Bar", {
		parentSection = expandable,
	})
	addon.functions.SettingsCreateCheckbox(cat, {
		var = "repBarEnabled",
		text = L["repBarEnabled"] or "Enable Reputation bar",
		desc = L["repBarDesc"] or "Shows your watched reputation as a customizable bar.",
		newTagID = "repBarEnabled",
		func = function(value)
	...
	addon.functions.SettingsCreateButton(cat, {
		var = "repBarCopyExperienceStyle",
		text = L["repBarCopyExperienceStyle"] or "Copy Experience bar style",
		buttonText = L["Copy"] or "Copy",
		desc = L["repBarCopyExperienceStyleDesc"] or "Copies size, textures, colors, border, and text style from the Experience bar without changing the Reputation bar anchor.",
		func = function()
			if addon.Aura and addon.Aura.ReputationBar and addon.Aura.ReputationBar.ApplyStyleFromExperienceBar then addon.Aura.ReputationBar:ApplyStyleFromExperienceBar() end
		end,
		parentSection = expandable,
	})
	addon.functions.SettingsCreateText(cat, "|cffffd700" .. (L["repBarEditModeHint"] or "Configure anchor, style, and text in Edit Mode.") .. "|r", {
		parentSection = expandable,
	})

	addon.functions.SettingsCreateHeadline(cat, L["HonorBar"] or "Honor Bar", {
		parentSection = expandable,
	})
	addon.functions.SettingsCreateCheckbox(cat, {
		var = "honorBarEnabled",
		text = L["honorBarEnabled"] or "Enable Honor bar",
		desc = L["honorBarDesc"] or "Shows your honor as a customizable bar.",
		newTagID = "honorBarEnabled",
		func = function(value)
			addon.db["honorBarEnabled"] = value and true or false
	...
	addon.functions.SettingsCreateButton(cat, {
		var = "honorBarCopyExperienceStyle",
		text = L["honorBarCopyExperienceStyle"] or "Copy Experience bar style",
		buttonText = L["Copy"] or "Copy",
		desc = L["honorBarCopyExperienceStyleDesc"] or "Copies size, textures, colors, border, and text style from the Experience bar without changing the Honor bar anchor.",
		func = function()
			if addon.Aura and addon.Aura.HonorBar and addon.Aura.HonorBar.ApplyStyleFromExperienceBar then addon.Aura.HonorBar:ApplyStyleFromExperienceBar() end
		end,
		parentSection = expandable,
	})
	addon.functions.SettingsCreateText(cat, "|cffffd700" .. (L["honorBarEditModeHint"] or "Configure anchor, style, and text in Edit Mode.") .. "|r", {
		parentSection = expandable,
	})
end
...
local function buildSettings()
	...
	createProgressBarSettings(cat, expandable)
...
if addon.Aura.functions.AddResourceBarsSettings then addon.Aura.functions.AddResourceBarsSettings() end
if addon.Aura.functions.AddResourceBarsProfileSettings then addon.Aura.functions.AddResourceBarsProfileSettings() end
