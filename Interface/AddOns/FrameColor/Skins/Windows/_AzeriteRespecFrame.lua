...
function skin:OnEnable()
  ...
  if C_AddOns.IsAddOnLoaded("Blizzard_AzeriteRespecUI") then
    ...
    local function onAddonLoaded(_, addOnName)
      if addOnName == "Blizzard_AzeriteRespecUI" then
        self:Apply(mainColor, 1)
...
function skin:Apply(mainColor, desaturation)
  -- Main frame.
  self:SkinNineSliced(AzeriteRespecFrame, mainColor, desaturation)

  for _, texture in pairs({
    AzeriteRespecFrame.ButtonFrame.ButtonBorder
  }) do
