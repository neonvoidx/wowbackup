...
function skin:OnEnable()
  if self:GetToggleState("follow_unit_class_or_reaction_color") then
    self:RegisterForEvent("PLAYER_FOCUS_CHANGED", skin.UpdateToUnitColor)
...
function skin:Apply(color, castBarColor, desaturation)
  -- Main frame.
  local texture = FocusFrame.TargetFrameContainer.FrameTexture
...
function skin:UpdateToUnitColor()
  local color = skin:GetUnitColor("focus")
  local texture = FocusFrame.TargetFrameContainer.FrameTexture
