...
local TransmogOutfitCollectionMixin = {
	DYNAMIC_EVENTS = {
...
BW_TransmogOutfitCollectionMixin = TransmogOutfitCollectionMixin
...
local ShowEquippedGearSpellFrameMixin = {};
...
local TransmogOutfitPopupMixin = {};
...
BW_TransmogWardrobeMixin = TransmogWardrobeMixin

function TransmogWardrobeMixin:OnLoad()
	TabSystemOwnerMixin.OnLoad(self);
	self:SetTabSystem(self.TabHeaders);
	self.itemsTabID = self:AddNamedTab(TRANSMOG_TAB_ITEMS, TransmogFrame.WardrobeCollection.TabContent.ItemsFrame);
...
local TransmogWardrobeItemsMixin = {
	DYNAMIC_EVENTS = {
	...
	COLLECTION_TEMPLATES = {
		["COLLECTION_ITEM"] = { template = "BW_TransmogItemModelTemplate", initFunc = BW_TransmogItemModelMixin.Init, resetFunc = BW_TransmogItemModelMixin.Reset }
	};
	WEAPON_DROPDOWN_WIDTH = 168;
};

BW_TransmogWardrobeItemsMixin = TransmogWardrobeItemsMixin
...
local TransmogWardrobeSetsMixin = {
	...
	COLLECTION_TEMPLATES = {
		["COLLECTION_SET"] = { template = "BW_TransmogSetModelTemplate", initFunc = BW_TransmogSetModelMixin.Init, resetFunc = BW_TransmogSetModelMixin.Reset }
	};
};
...
function TransmogWardrobeSetsMixin:GetCurrentTransmogInfoCallback()
	return 	TransmogFrame.WardrobeCollection.TabContent.SetsFrame.wardrobeCollection.GetCurrentTransmogInfoCallback();
end


local TransmogWardrobeCustomSetsMixin = {
	DYNAMIC_EVENTS = {
	...
	COLLECTION_TEMPLATES = {
		["COLLECTION_CUSTOM_SET2"] = { template = "BW_TransmogCustomSetModelTemplate", initFunc = BW_TransmogCustomSetModelMixin.Init, resetFunc = BW_TransmogCustomSetModelMixin.Reset }
	};
};
...
function TransmogWardrobeCustomSetsMixin:GetFirstMatchingCustomSetID()
	...
	local customSets = C_TransmogCollection.GetCustomSets();
		--local customSets = addon.OutfitDB.char.outfits --[i + 9 * (self.Page - 1)]
...
--[[
local TransmogWardrobeSituationsMixin = {
	DYNAMIC_EVENTS = {
		"VIEWED_TRANSMOG_OUTFIT_SITUATIONS_CHANGED"
	};
};
