			...
			if not lib.layoutCache[index] then
				for _, callback in next, lib.anonCallbacksCreate do
					securecallfunction(
						...
						layout.layoutName,
						index,
						lib._layoutCopySource and lib._layoutCopySource.layoutName
					)
				end
			end
		end
	...
	hooksecurefunc(EditModeManagerFrame, "SelectSystem", function(_, systemFrame)
		if lib.hookVersion == MINOR then
			...
			local isKnownSubSystem = subSystemID
				and (
					(lib.subSystemSettings[systemID] and lib.subSystemSettings[systemID][subSystemID])
					or (lib.subSystemButtons[systemID] and lib.subSystemButtons[systemID][subSystemID])
				)
